...
import { X, Save, Image, FileText, Palette, Link as LinkIcon, Tag, User, ChevronDown, Lock } from 'lucide-react';
import comicService from '../../services/comicService';

export default function EditComicModal({ isOpen, onClose, onSuccess, initialData }) {
    ...
    if (isOpen) {
      ...
      if (initialData) {
        setFormData({
          ...
          publishStatus: initialData.publishStatus || 'ONGOING',
          originCountry: initialData.originCountry || 'KOREA',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.genreIds && formData.genreIds.length > 0) {
        ...
        // sending empty genreIds
        data.append('genreIds', '');
      }
      ...
      await comicService.updateComic(initialData.id, data);
      onSuccess?.();
      onClose();
    } catch (err) {
      const msg = err.response?.data?.message || 'Đã xảy ra lỗi khi cập nhật truyện. Vui lòng thử lại.';
      setError(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center">
      ...
      <div className="relative bg-dark-900 border border-dark-700/50 rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto animate-scale-in">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-dark-700/50 sticky top-0 bg-dark-900 z-10">
          <h2 className="text-lg font-semibold text-white">Chỉnh sửa thông tin truyện</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-2 opacity-70">
            ...
            <input
              ...
              value={formData.slug}
              readOnly
              className="w-full px-4 py-2.5 rounded-xl bg-dark-800/50 border border-dark-700/50 text-sm text-dark-400 font-mono cursor-not-allowed"
            />
            <p className="text-[11px] text-dark-600">
              Slug không thể thay đổi sau khi tạo để tránh lỗi đường dẫn và lưu trữ.
            </p>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-dark-300 flex items-center gap-2">
              <Image size={14} />
              Ảnh bìa (Thumbnail) - Không chọn để giữ nguyên cữ
            </label>
            ...
            {!thumbnailPreview ? (
              ...
              >
                <div className="w-12 h-12 rounded-xl bg-dark-800 group-hover:bg-primary-500/10 flex items-center justify-center transition-all">
                  <Image size={22} className="text-dark-500 group-hover:text-primary-400 transition-colors" />
                </div>
                <div>
                  <p className="text-sm text-dark-400 group-hover:text-dark-300 transition-colors">Click để chọn ảnh bìa mới</p>
              ...
              <div className="relative group">
                ...
                <p className="text-xs text-dark-500 mt-2 truncate">{thumbnail?.name || 'Ảnh bìa hiện tại'}</p>
              </div>
            )}
