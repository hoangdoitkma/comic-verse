...
public class NovelDetailFragment extends Fragment implements CommentAdapter.OnCommentInteractionListener {
    ...
    private void setupViews() {
        ...
        binding.btnViewAllChapters.setOnClickListener(v -> {
            ...
            String title = state.getComic() != null ? state.getComic().getTitle() : null;
            showChapterListBottomSheet(state.getChapters(), state.getComic() != null ? state.getComic().getId() : args.getComicId(), title);
        });
    }
    ...
    private void bindComicData(ComicDetailEntity comic) {
        ...
        binding.txtChaptersTitle.setText("Chương truyện chữ mới");
        binding.txtSimilarTitle.setText("Truyện chữ cùng thể loại");
    ...
    private void showChapterListBottomSheet(List<ChapterItem> chapters, int comicId, String comicTitle) {
        ...
        chapterListAdapter = new BottomSheetChapterAdapter(chapters, chapterId -> {
            ...
            com.example.comicversev1.NavGraphDirections.ActionGlobalTextNovelReader action =
                    com.example.comicversev1.NavGraphDirections.actionGlobalTextNovelReader(chapterId, comicId);
            action.setComicTitle(comicTitle);
            NavHostFragment.findNavController(this).navigate(action);
        });
        if (savedChapterId > 0) {
        ...
        if (languageChip != null) {
            languageChip.setText("Truyện chữ");
        }
